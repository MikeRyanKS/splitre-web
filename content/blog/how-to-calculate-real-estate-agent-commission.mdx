...
This guide works through every layer of the calculation, from the straightforward to the genuinely tricky. Want to skip straight to a number? Run a real deal through the [free commission split calculator](/real-estate-commission-split-calculator) as you read.
...
The most common ones are **E&O insurance** (a per-deal fee, often $100 to $300, that funds the brokerage's professional liability coverage), **franchise fees** for brokerages affiliated with a national brand (typically 5 to 8% of GCI), and **transaction coordinator fees** if the brokerage passes those through before splitting.
...
A **graduated-to-cap** structure is probably the most common in independent brokerages. The agent earns at a split until they've paid the broker a fixed annual maximum (the cap), at which point they keep 100% of every subsequent deal. See [Creating a Commission Plan](/docs/creating-commission-plans) for how each of these structures is configured.
...
The cap balance is simply the running total of what the broker has collected so far in the plan year. It has to be accurate before every deal. The moment you enter a deal with a wrong balance, the calculation is wrong, and the error follows the agent's records forward. See [The Real Estate Commission Cap, Explained](/blog/real-estate-commission-cap-explained) for a full breakdown of how brokers get this wrong in practice.
...
If another agent referred the client, a referral fee typically comes off the agent's gross commission (not the net) before the internal split is applied. Referral fees are usually 20 to 25% and go to the referring party first.
...
SplitRE is built specifically for this. Try the [commission split calculator](/real-estate-commission-split-calculator) free, or see [how it works end to end](/features).
