...
related: ["how-to-calculate-real-estate-agent-commission", "eliminate-commission-spreadsheets"]
---

A real estate commission cap is the maximum dollar amount an agent pays their broker in a single plan year. Once that amount has been collected, the agent keeps 100% of the net commission on every deal for the rest of the year. The broker's cut simply stops.

Ask an agent what they look for when evaluating a brokerage, and the cap comes up early. It's a compelling concept: pay the broker a fixed maximum each year, then keep everything after that. It's one of the few structural advantages independent brokerages hold over larger franchises, and it's also one of the most frequently mismanaged parts of brokerage accounting, which is exactly why getting the mechanics right matters.
...
The cap balance, sometimes called the [cap ledger](/docs/glossary#cap-ledger), is the running total of what the broker has collected from an agent's deals since the start of the plan year. It needs to be updated after every deal and checked before every new deal is processed.
...
SplitRE tracks every agent's cap ledger automatically and flips the split the moment a deal crosses it. See [Setting Up Agent Caps](/docs/setting-up-agent-caps) for how it's configured, or [try a cap-crossing deal](/real-estate-commission-split-calculator) in the free calculator.
